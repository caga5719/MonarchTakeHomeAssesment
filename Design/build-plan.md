...
2. **Write `schema.sql`** — DDL for all four tables:
   ...
   - `invoices (id, invoice_number UNIQUE, property_code FK, invoice_gl_code FK, invoice_date, due_date, purchaser, po_number, subtotal, tax, total_amount, filename)`
   - `line_items (id, invoice_id FK, line_number, description, asin, quantity, unit_price, subtotal, tax_rate, assigned_gl_code FK, assigned_gl_desc, classification_note)`
...
5. **Initialize React frontend with Vite**
   ...
   ```
   Create a minimal `App.tsx` with placeholder routes so the dev server starts without errors.

6. **Write `.env.example`**: `ANTHROPIC_API_KEY=sk-ant-...`

### Done When
- `python backend/app/seed.py` runs without errors
- `SELECT COUNT(*) FROM gl_codes` → 1406
...
1. **Write `ingest.py`**:
   ...
   - Print a summary: `Processed N invoices, M line items, K skipped (already in DB)`
...
2. **Handle edge cases**:
   ...
   - Discount rows: detect by negative subtotal or `Promotions` keyword; insert into DB with `quantity=NULL` and no GL classification
   - Property code not in `properties` table: insert the invoice anyway, log a warning
...
### Schema addition (add to `schema.sql` before running this phase)

```sql
-- Add to line_items table definition in schema.sql:
