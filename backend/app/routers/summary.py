...
def summary(db=Depends(get_db)):
    ...
    total_line_items = db.execute("SELECT COUNT(*) AS c FROM line_items").fetchone()["c"]
    ...
    needs_review_count = db.execute(
        "SELECT COUNT(*) AS c FROM line_items WHERE needs_review = 1"
    ).fetchone()["c"]

    top_gl_row = db.execute("""
        SELECT
...
def mismatches(db=Depends(get_db)):
...
def needs_review(db=Depends(get_db)):
