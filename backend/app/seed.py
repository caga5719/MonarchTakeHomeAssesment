...
def seed_gl_codes(conn) -> int:
    ...
    conn.executemany(
        "INSERT OR REPLACE INTO gl_codes (scode, sdesc) VALUES (?, ?)",
        data,
    )
    conn.commit()
    wb.close()
...
def seed_properties(conn) -> int:
    ...
    # Build dict keyed by normalized yardi_code → {website_id}
    owned: dict[str, dict] = {}
    ...
    # Headers: Website ID, Yardi Code, Property Name, LegalEntityName, Status,
    #          Unit Count, Total Capital, Acquisition Date, Sale Date, Address,
    #          City, State, Zip, Phone, Metro Area, PelitonID
    ...
    for yardi_code, base in owned.items():
        ...
        records.append((
            ...
            extra.get("name"),
            extra.get("state"),
            extra.get("unit_count"),
        ))

    conn.executemany(
        """
        INSERT OR REPLACE INTO properties (yardi_code, website_id, name, state, unit_count)
        VALUES (?, ?, ?, ?, ?)
        """,
        records,
    )
...
def main() -> None:
    ...
    with get_connection() as conn:
        ...
        gl_count = seed_gl_codes(conn)
        print(f"  Inserted/replaced {gl_count} GL codes")

        print("Seeding properties...")
        prop_count = seed_properties(conn)
